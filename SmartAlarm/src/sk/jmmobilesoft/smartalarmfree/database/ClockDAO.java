...
import sk.jmmobilesoft.smartalarmfree.database.ClockContract.ClockModel;
import sk.jmmobilesoft.smartalarmfree.model.Clock;
...
public class ClockDAO {
	...
	private static final String SQL_CREATE_TABLE = "CREATE TABLE "
			...
			+ ClockModel.COLUMN_NAME_CLOCK_VIBRATE + " INTEGER,"
	...
	public long createClock(SQLiteDatabase db, Clock clock) {
		...
		long id = db.insert(ClockModel.TABLE_NAME, null, values);
		db.close();
		return id;
	}
	...
	public long updateClock(SQLiteDatabase db, Clock clock) {
		ContentValues values = populateClockContent(clock);
		long id = db.update(ClockModel.TABLE_NAME, values, ClockModel._ID
	...
	public int deleteClock(SQLiteDatabase db, long id) {
		int rows = db.delete(ClockModel.TABLE_NAME, ClockModel._ID + " =?",
	...
	private Clock populateClockModel(Cursor c) {
		...
		clock.setActive(c.getInt(c
				...
				: false);
		clock.setVibrate(c.getInt(c
				.getColumnIndex(ClockModel.COLUMN_NAME_CLOCK_VIBRATE)) == 1 ? true
				: false);
	...
	private ContentValues populateClockContent(Clock clock) {
		...
		values.put(ClockModel.COLUMN_NAME_CLOCK_ENABLED, clock.isActive() ? 1
				: 0);
		values.put(ClockModel.COLUMN_NAME_CLOCK_VIBRATE, clock.isVibrate() ? 1
				: 0);
